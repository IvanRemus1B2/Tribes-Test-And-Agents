...
public class TribesDiffHeuristicV2 implements StateHeuristic {
    private int playerID;
    ArrayList<Integer> allIds;

    public TribesDiffHeuristicV2( int playerID , ArrayList<Integer> allIds ) {
        this.allIds = allIds;
    ...
    private double score( GameState gsOld , GameState gsNew , int playerId ) {
        ...
        difference += 0.025 * ( score - gsOld.getScore( playerId ) );
        difference += 4 * ( cities - gsOld.getCities( playerId ).size() );
        difference += 3.5 * ( units - gsOld.getUnits( playerId ).size() );
        difference += 2.5 * ( kills - gsOld.getNKills( playerId ) );
