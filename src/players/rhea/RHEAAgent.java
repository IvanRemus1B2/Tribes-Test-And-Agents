...
//        System.out.println(currentTurn + ":" + actionInTurn + ": toExecute: " + toExecute);
    ...
    private Action getRandomAction( GameState gs ) {
        ...
        var availableActions = this.allGoodActions( gs , AlgParams.excludedActions );
        return availableActions.get( m_rnd.nextInt( availableActions.size() ) );
    }
    ...
    private ArrayList<Genome> nextGeneration( GameState gs ) {
        ...
        while (newPop.size() < params.POP_SIZE) {
            Genome g;
            // TODO:Since the population size is most of the time bigger than 1,we will
            //  never get to add mutating individuals to the population
